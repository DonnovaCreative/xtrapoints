...
The marketing portal and Ambassador Toolkit use the same XtraPoint navy sidebar with lime active states with a top product switcher. School logos and theme colors belong to school pages and collateral; the application navigation identifies the current school with text. The Marketing Portal's former Resource library is labeled Marketing materials.

The toolkit sidebar lists its six resources. Resource categories and article topics expand beneath their resource in the sidebar; the current topic and its containing branches are revealed on navigation. Desktop collapse and width preferences persist across pages and both native products. Drag the separator or use its arrow keys to resize; the compact edge expands the sidebar. The repeated private-access banner is removed. `/resources` redirects to `/resources/ambassador-toolkit` while the toolkit is the only shared guide collection. Main-site articles, search and downloads keep the existing private access gate; the navigation redesign does not make them public. The public header now includes Partner login on desktop and in the mobile menu, pointing to `/portal` and its existing sign-in/workspace flow.
